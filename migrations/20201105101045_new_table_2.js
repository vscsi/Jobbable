exports.up = function(knex) {
    return knex.schema
        ...
        .createTable('login_users', (table) => {
            ...
            table.string('employers_status').unsigned();
            table.foreign('employers_status').references('employers.status');
            table.string('employees_status').unsigned();
            table.foreign('employees_status').references('employees.status');
        })
};

exports.down = function(knex) {
    return knex.schema.dropTable('jobs_skilltag').dropTable('employee_skilltag').dropTable('skilltag').dropTable('login_users');
};